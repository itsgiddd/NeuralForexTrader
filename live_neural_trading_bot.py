...
try:
    from enhanced_neural_architecture import EnhancedNeuralArchitecture
...
class LiveNeuralTradingBot:
    ...
    def __init__(self, 
        ...
        logger.info(f"🚀 Live Neural Trading Bot Initialized")
    ...
    def get_market_data(self, symbol: str) -> Optional[MarketData]:
        ...
        try:
            ...
            if symbol_info is None:
                logger.warning(f"⚠️ Symbol info not found: {symbol}")
                return None
            
            # Get quote data
            quote = mt5.symbol_info_tick(symbol)
            if quote is None:
                logger.warning(f"⚠️ Quote data not found: {symbol}")
                return None
    ...
    def generate_neural_signal(self, market_data: MarketData, account_info: AccountInfo) -> Optional[TradeSignal]:
        ...
        try:
            ...
            result = self.neural_brain.think(
                ...
                symbol_info=mt5.symbol_info(market_data.symbol)
            )
    ...
    def _generate_simple_signal(self, market_data: MarketData) -> TradeSignal:
        ...
        try:
            ...
            closes = market_data.h1_data['close'].values
            if len(closes) < 10:
                return TradeSignal(
                    symbol=market_data.symbol,
                    action=TradeResult.HOLD,
            ...
            if ma5 > ma10:
                confidence = min(abs(ma5 - ma10) / ma10 * 10, 0.8)
                action = TradeResult.BUY
                stop_loss = market_data.bid - (market_data.spread * 2)
                take_profit = market_data.ask + (market_data.spread * 4)
            elif ma5 < ma10:
                confidence = min(abs(ma10 - ma5) / ma10 * 10, 0.8)
                action = TradeResult.SELL
                stop_loss = market_data.ask + (market_data.spread * 2)
                take_profit = market_data.bid - (market_data.spread * 4)
            else:
                return TradeSignal(
                    symbol=market_data.symbol,
                    action=TradeResult.HOLD,
            ...
            return TradeSignal(
                ...
                action=action,
                confidence=confidence,
                lot_size=0.01,  # Fixed small lot for demo
                stop_loss=stop_loss,
                take_profit=take_profit,
                reason=f"Simple momentum signal (MA5: {ma5:.5f}, MA10: {ma10:.5f})",
                timestamp=datetime.now()
            )
    ...
    def execute_trade(self, signal: TradeSignal, account_info: AccountInfo) -> bool:
        ...
        try:
            ...
            if not self._check_risk_conditions(signal, account_info):
                logger.warning(f"⚠️ Trade rejected due to risk management: {signal.symbol}")
                return False
            ...
            if lot_size <= 0:
                logger.warning(f"⚠️ Invalid lot size calculated: {lot_size}")
                return False
    ...
    def _check_risk_conditions(self, signal: TradeSignal, account_info: AccountInfo) -> bool:
        ...
        try:
            ...
            if account_info.margin_level < 200:
                logger.warning("⚠️ Low margin level")
                return False
    ...
    def _calculate_position_size(self, signal: TradeSignal, account_info: AccountInfo, symbol_info, base_lot: float = 0.01) -> float:
        ...
        try:
            ...
            risk_factor = self.max_risk_per_trade
            balance_risk = account_info.balance * risk_factor
            
            # Calculate pip value
            tick_value = symbol_info.trade_tick_value
            pip_distance = abs(signal.stop_loss - (signal.stop_loss + (1 if signal.action == TradeResult.BUY else -1) * symbol_info.point))
    ...
    def update_performance_metrics(self):
        ...
        try:
            ...
            if total_trades > 0:
                ...
                winning_profit = self.performance_metrics['total_profit']
                if winning_profit > 0:
                    losing_trades = self.performance_metrics['losing_trades']
                    if losing_trades > 0:
                        avg_win = winning_profit / self.performance_metrics['winning_trades']
                        avg_loss = abs(winning_profit) / losing_trades  # Simplified
                        self.performance_metrics['profit_factor'] = avg_win / avg_loss if avg_loss > 0 else 0
            ...
            try:
                ...
                for symbol in self.symbols:
                    try:
                        ...
                        # Log signal
                        if signal.action != TradeResult.HOLD:
                            logger.info(f"🎯 Signal: {signal.action.value} {signal.symbol} - Confidence: {signal.confidence:.1%}")
                            logger.info(f"   Reason: {signal.reason}")
...
def main():
    ...
    bot = LiveNeuralTradingBot(
        ...
        symbols=['EURUSD', 'GBPUSD', 'USDJPY']  # Add your preferred symbols
    )
